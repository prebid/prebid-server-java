...
public class RequestRuleParser {
    ...
    public Map<Stage, Rule<BidRequest>> parse(ObjectNode accountConfig) {
        ...
        return parsedConfig.getRuleSets().stream()
                ...
                .collect(Collectors.toMap(
                        e->e.getKey(), entry -> CompositeRule.of(entry.getValue())));
    }
    ...
    private static Rule<BidRequest> toRule(ModelGroupConfig modelGroupConfig) {
        final Schema<BidRequest> schema = parseSchema(modelGroupConfig);
    ...
    private static Schema<BidRequest> parseSchema(ModelGroupConfig modelGroupConfig) {
        final List<SchemaFunctionHolder<BidRequest>> schemaFunctions = modelGroupConfig.getSchema().stream()
                .map(config -> SchemaFunctionHolder.of(
                        config.getFunction(),
                        RequestSchema.schemaFunctionByName(config.getFunction()),
                        config.getArgs()))
                .toList();
