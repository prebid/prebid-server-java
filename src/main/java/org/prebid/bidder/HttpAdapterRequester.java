...
import org.prebid.exception.InvalidRequestException;
...
public class HttpAdapterRequester implements BidderRequester {
     ...
     * {@link Bidder} to make call to legacy adapters. After response was returned it converts it
    ...
    public Future<BidderSeatBid> requestBids(BidRequest bidRequest, GlobalTimeout timeout) {
        ...
        final Result<org.prebid.model.Bidder> bidderWithErrors;
        try {
    ...
    private Result<Bidder> toBidder(BidRequest bidRequest) {
        ...
        if (adUnitBids.size() > 0) {
            return Result.of(org.prebid.model.Bidder.of(adapter.code(), adUnitBids), errors);
        }
    ...
    private Result<List<BidderBid>> toBidderBids(BidderResult bidderResult) {
        ...
        final List<BidderError> errors = new ArrayList<>();
        for (org.prebid.model.response.Bid bid : bidderResult.getBids()) {
    ...
    /**
     * Converts {@link org.prebid.model.response.Bid} to {@link Result<BidType>}. In case {@link MediaType} is not
     * defined in {@link org.prebid.model.response.Bid} or incorrect {@link MediaType} value, {@link Result} with
     * error list ad null value will be returned.
     */
    private Result<BidType> toBidType(org.prebid.model.response.Bid bid) {
    ...
    /**
     * Converts {@link org.prebid.model.response.Bid} to {@link Bid}.
     */
    private Bid toOrtbBid(org.prebid.model.response.Bid bid) {
