...
public class AxonixBidder implements Bidder<BidRequest> {
    ...
    private static final String RESERVE_ENDPOINT_URL = "https://openrtb-us-east-1.axonix.com/supply/prebid-server/";
    ...
    public Result<List<HttpRequest<BidRequest>>> makeHttpRequests(BidRequest request) {
        ...
        try {
            extImpAxonix = mapper.mapper().convertValue(firstImp.getExt(), AXONIX_EXT_TYPE_REFERENCE).getBidder();
        } catch (IllegalArgumentException e) {
            return Result.withError(BidderError.badInput(e.getMessage()));
        }

        final String uri = StringUtils.isNotEmpty(endpointUrl) ? endpointUrl
                : HttpUtil.encodeUrl(String.format("%s/%s", RESERVE_ENDPOINT_URL, extImpAxonix.getSupplyId()));
