package org.prebid.server.bidder.dianomi;
...
import org.prebid.server.proto.openrtb.ext.request.dianomi.ExtImpDianomi;
...
public class DianomiBidder implements Bidder<BidRequest> {

    private static final String REQUEST_EXT_PT = "pt";
    private static final TypeReference<ExtPrebid<?, ExtImpDianomi>> DIANOMI_EXT_TYPE_REFERENCE =
            new TypeReference<>() {
            };
    ...
    public DianomiBidder(String endpointUrl, JacksonMapper mapper) {
    ...
    public Result<List<HttpRequest<BidRequest>>> makeHttpRequests(BidRequest bidRequest) {
        ...
        for (Imp imp : bidRequest.getImp()) {
            final ExtImpDianomi extImpDianomi;
            try {
                extImpDianomi = parseImpExt(imp);
            } catch (PreBidException e) {
                errors.add(BidderError.badInput(e.getMessage()));
                continue;
            }

            modifiedImps.add(imp.toBuilder().tagid(extImpDianomi.getSmartAdId()).build());
            priceType = StringUtils.defaultIfEmpty(priceType, extImpDianomi.getPriceType());
        }
    ...
    private ExtImpDianomi parseImpExt(Imp imp) {
        try {
            return mapper.mapper().convertValue(imp.getExt(), DIANOMI_EXT_TYPE_REFERENCE).getBidder();
