    ...
    public void openrtb2VideoShouldRespondWithBidsFromAppnexus() throws IOException, JSONException {
        ...
        WIRE_MOCK_RULE.stubFor(post(urlPathEqualTo("/cache"))
                .withRequestBody(equalToBidCacheRequest(
                        jsonFrom("openrtb2/appnexus/test-video-cache-request.json")))
                .willReturn(aResponse()
                        .withTransformers("cache-response-transformer")
                        .withTransformerParameter("matcherName",
                                "openrtb2/appnexus/test-video-cache-response-matcher.json")));

        // when
        final Response response = responseFor("openrtb2/appnexus/test-video-appnexus-request.json",
