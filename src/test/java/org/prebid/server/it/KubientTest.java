...
public class KubientTest extends IntegrationTest {
    ...
    public void openrtb2AuctionShouldRespondWithBidsFromKubient() throws IOException, JSONException {
        ...
        WIRE_MOCK_RULE.stubFor(post(urlPathEqualTo("/kubient-exchange"))
                .withRequestBody(equalToJson(jsonFrom("openrtb2/kubient/test-kubient-bid-request.json")))
                .willReturn(aResponse().withBody(jsonFrom("openrtb2/kubient/test-kubient-bid-response.json"))));

        // when
        final Response response = responseFor("openrtb2/kubient/test-auction-kubient-request.json",
