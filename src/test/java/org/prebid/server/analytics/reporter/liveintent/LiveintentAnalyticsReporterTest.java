...
public class LiveintentAnalyticsReporterTest extends VertxTest {
    ...
    public void shouldSendAllBidsToLiveIntent() {
        // given
        final HttpClientResponse mockResponse = mock(HttpClientResponse.class);
        when(httpClient.post(anyString(), anyString(), anyLong())).thenReturn(Future.succeededFuture(mockResponse));
        ...
        assertThat(pbsjBids)
                .isEqualTo(List.of(PbsjBid.builder().bidId("bid-id").price(BigDecimal.ONE).adUnitId("ad-unit-id")
                        .enriched(true).currency("USD").treatmentRate(0.5f).timestamp(0L).partnerId("pbsj").build()));
    }
    ...
    public void shouldSendAllBidsToLiveIntentNotEnriched() {
        ...
        assertThat(pbsjBids)
                .isEqualTo(List.of(PbsjBid.builder().bidId("bid-id").price(BigDecimal.ONE).adUnitId("ad-unit-id")
                        .enriched(false).currency("USD").treatmentRate(0.5f).timestamp(0L).partnerId("pbsj").build()));
    }
    ...
    public void shouldSendAllBidsToLiveIntentNoTreatmentRate() {
        ...
        assertThat(pbsjBids)
                .isEqualTo(List.of(PbsjBid.builder().bidId("bid-id").price(BigDecimal.ONE).adUnitId("ad-unit-id")
                        .enriched(false).currency("USD").timestamp(0L).treatmentRate(null).partnerId("pbsj").build()));
    }
    ...
    private AuctionEvent buildEvent(Boolean isEnriched, Boolean withTags) {
        final HookId hookId = HookId.of("pb-liveintent-omni-channel-identity-enrichment-hook",
                "pb-liveintent-omni-channel-identity-enrichment-hook");

        final ObjectNode treatmentRateNode = ObjectMapperProvider.mapper().createObjectNode().put("treatmentRate",
                0.5f);

        final AppliedToImpl appliedTo = AppliedToImpl.builder().bidIds(List.of("bid-id")).impIds(List.of("imp-id"))
                .bidders(List.of("pbsj")).request(true).response(true).build();

        final ResultImpl result = ResultImpl.of("treatmentRate", treatmentRateNode, appliedTo);

        final ActivityImpl enrichmentRate = ActivityImpl.of("liveintent-treatment-rate", "0.5", List.of(result));

        final List<ActivityImpl> enriched = isEnriched
                ? List.of(ActivityImpl.of("liveintent-enriched", "success", List.of()))
                : List.of();

        final HookExecutionOutcome hookExecutionOutcome = HookExecutionOutcome.builder().hookId(hookId)
                .executionTime(100L).status(ExecutionStatus.success)
                .analyticsTags(TagsImpl.of(withTags ? ListUtil.union(List.of(enrichmentRate), enriched) : List.of()))
                .action(null).build();

        final StageExecutionOutcome stageExecutionOutcome = StageExecutionOutcome.of("auction-request",
