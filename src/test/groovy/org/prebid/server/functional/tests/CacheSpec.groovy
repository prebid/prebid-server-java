...
class CacheSpec extends BaseSpec {
    ...
    def "PBS should update prebid_cache.creative_size.json metric when json creative is received"() {
        ...
        def initialValue = getCurrentMetricValue(defaultPbsService, MetricsUtil.Cache.requestsOk())
        ...
        assert metrics[MetricsUtil.Cache.requestsOk()] == initialValue + 1
        assert metrics[MetricsUtil.Cache.creativeSizeJson()] == creativeSize

        and: "account.<account-id>.prebid_cache.creative_size.json should be update"
        assert metrics[MetricsUtil.Cache.accountRequestsOk(bidRequest.accountId)] == 1
        assert metrics[MetricsUtil.Cache.accountCreativeSizeJson(bidRequest.accountId)] == creativeSize
    }
    ...
    def "PBS should update prebid_cache.creative_size.xml metric when video bid and xml creative is received"() {
        ...
        def metrics = defaultPbsService.sendCollectedMetricsRequest()
        assert metrics[MetricsUtil.Cache.requestsOk()] == initialValue + 1
        assert metrics[MetricsUtil.Cache.creativeSizeXml()] == creativeSize

        and: "account.<account-id>.prebid_cache.creative_size.json should be update"
        assert metrics[MetricsUtil.Cache.accountRequestsOk(bidRequest.accountId)] == 1
        assert metrics[MetricsUtil.Cache.accountCreativeSizeXml(bidRequest.accountId)] == creativeSize
    }
    ...
    def "PBS should cache banner bids with cache key that include account and datacenter short name when append-trace-info-to-cache-id enabled"() {
        ...
        assert metrics[MetricsUtil.Cache.accountCreativeTtlJson(bidRequest.accountId)] == bannerHostTtl
        assert metrics[MetricsUtil.Cache.accountRequestsOk(bidRequest.accountId)] == 1
    ...
    def "PBS should cache video bids with cache key that include account and datacenter short name when append-trace-info-to-cache-id enabled"() {
        ...
        def metrics = pbsService.sendCollectedMetricsRequest()
        assert metrics[MetricsUtil.Cache.accountCreativeTtlJson(bidRequest.accountId)] == videoHostTtl
        assert metrics[MetricsUtil.Cache.accountCreativeTtlXml(bidRequest.accountId)] == videoHostTtl
        assert metrics[MetricsUtil.Cache.accountRequestsOk(bidRequest.accountId)] == 1

        and: "PBS should include metrics prebid_cache_creative.{xml,json}.creative.ttl for general"
        assert metrics[MetricsUtil.Cache.creativeTtlXml()] == videoHostTtl
        assert metrics[MetricsUtil.Cache.creativeTtlJson()] == videoHostTtl
