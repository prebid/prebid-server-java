...
class GppTransmitUfpdActivitiesSpec extends PrivacyBaseSpec {
    ...
    def "PBS auction call when privacy regulation don't match custom requirement should leave UFPD fields in request"() {
        ...
        def accountGppConfig = new AccountGppConfig().tap {
            ...
            it.enabled = true
            it.moduleConfig = ModuleConfig.getDefaultModuleConfig(new ActivityConfig([TRANSMIT_UFPD], accountLogic))
        }
    ...
    def "PBS auction call when custom privacy regulation have invalid setup should leave UFPD fields in request with warning and add alert metric"() {
        ...
        def accountGppConfig = new AccountGppConfig().tap {
            ...
            it.config = new SidsConfig().tap { it.skipSids = [] }
            it.enabled = true
            it.moduleConfig = ModuleConfig.getDefaultModuleConfig(new ActivityConfig([TRANSMIT_UFPD], restrictedRule))
        }
