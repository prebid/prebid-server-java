...
class CoppaSpec extends PrivacyBaseSpec {
    ...
    def "PBS shouldn't mask device and user fields for amp request when coppa = 0 was passed"() {
        ...
        assert !metrics[MetricsUtil.Privacy.adapterDisallowedActivityCount(GENERIC, TRANSMIT_EIDS)]
        assert !metrics[MetricsUtil.Privacy.adapterDisallowedActivityCount(GENERIC, TRANSMIT_PRECISE_GEO)]
        assert !metrics[MetricsUtil.Privacy.accountDisallowedActivityCount(ampStoredRequest.accountId, TRANSMIT_UFPD)]
        assert !metrics[MetricsUtil.Privacy.accountDisallowedActivityCount(ampStoredRequest.accountId, TRANSMIT_EIDS)]
        assert !metrics[MetricsUtil.Privacy.accountDisallowedActivityCount(ampStoredRequest.accountId, TRANSMIT_PRECISE_GEO)]
        assert !metrics[MetricsUtil.Privacy.requestDisallowedActivityCount(TRANSMIT_UFPD)]
        assert !metrics[MetricsUtil.Privacy.requestDisallowedActivityCount(TRANSMIT_EIDS)]
        assert !metrics[MetricsUtil.Privacy.requestDisallowedActivityCount(TRANSMIT_PRECISE_GEO)]
    }
