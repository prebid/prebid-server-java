...
class GppTransmitTidActivitiesSpec extends PrivacyBaseSpec {
    ...
    def "PBS auction should remove bidRequest.(source/imp[0].ext).tid and don't change schain in request when ext.prebid.createTids=null and activity disallowed and schain specified in reques"() {
        ...
        def metrics = activityPbsService.sendCollectedMetricsRequest()
        assert metrics[MetricsUtil.Privacy.requestDisallowedActivityCount(TRANSMIT_TID)] == 1
        assert metrics[MetricsUtil.Privacy.accountDisallowedActivityCount(accountId, TRANSMIT_TID)] == 1
        assert metrics[MetricsUtil.Privacy.adapterDisallowedActivityCount(BidderName.GENERIC, TRANSMIT_TID)] == 1
    }
