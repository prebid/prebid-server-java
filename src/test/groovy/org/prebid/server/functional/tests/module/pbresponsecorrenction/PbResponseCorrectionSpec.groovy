...
class PbResponseCorrectionSpec extends ModuleBaseSpec {
    ...
    def "PBS should modify response when requested video impression respond with invalid adm VAST keyword and disabled cache config"() {
        ...
        assert pbsServiceWithMultipleModules.isContainLogsByValue(responseCorrectionLog)
    ...
    private static Account accountConfigWithResponseCorrectionModule(BidRequest bidRequest,
        ...
        getAccountWithModuleConfig(bidRequest.accountId, [(ALL_PROCESSED_BID_RESPONSES): [PB_RESPONSE_CORRECTION]]).tap {
