...
class ProfileSpec extends BaseSpec {
    ...
    def "PBS should fail auction when fail-on-unknown-profile enabled and profile is missing"() {
        ...
        verifyAll(response.ext.errors[PREBID]) {
            ...
            it.errorMessage == [INVALID_REQUEST_PREFIX + NO_IMP_PROFILE_MESSAGE.formatted(invalidProfileId)]
        }
    ...
    def "PBS shouldn't validate profiles and imp before margining"() {
        ...
        verifyAll(response.ext.errors[PREBID]) {
            it.code == [BidderErrorCode.GENERIC]
            it.errorMessage == ['Invalid request format: request.imp[0].banner.format[0] must define a valid "h" and "w" properties']
        }
    }
    ...
    def "PBS should throw exception when profiles are not configured and request contain profileId"() {
        ...
        verifyAll(response.ext.errors[PREBID]) {
            it.code == [BidderErrorCode.GENERIC]
            it.errorMessage == [INVALID_REQUEST_PREFIX + CONFIG_ERROR_MESSAGE]
        }
