...
class CacheStorageSpec extends ModuleBaseSpec {
    ...
    private final PrebidServerService prebidServerStoredCacheService = pbsServiceFactory.getService(getOptableTargetingSettings() + CACHE_STORAGE_CONFIG)
    ...
    def cleanupSpec() {
        pbsServiceFactory.removeContainer(getOptableTargetingSettings() + CACHE_STORAGE_CONFIG)
    }
    ...
    private static getTargetingResultSize(TargetingResult result) {
        encodeBase64(encode(result).bytes).size()
    }
}
