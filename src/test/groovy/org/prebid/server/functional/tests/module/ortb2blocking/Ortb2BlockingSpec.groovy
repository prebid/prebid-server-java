...
class Ortb2BlockingSpec extends ModuleBaseSpec {
    ...
    def "PBS shouldn't send original inappropriate ortb2 attribute to bidder when blocking is enabled"() {
        ...
        and: "Account in the DB with blocking configuration"
        def ortb2BlockingAttributeConfig = Ortb2BlockingAttributeConfig.getDefaultOrtb2BlockingAttributeConfig([ortb2Attributes], attributeName).tap {
            enforceBlocks = true
        }
    ...
    def "PBS should send only not matched ortb2 attribute to bidder when blocking is enabled"() {
        ...
        and: "Account in the DB with blocking configuration"
        def ortb2BlockingAttributeConfig = Ortb2BlockingAttributeConfig.getDefaultOrtb2BlockingAttributeConfig([disallowedOrtb2Attributes], attributeName).tap {
            enforceBlocks = true
        }
        def account = getAccountWithOrtb2BlockingConfig(bidRequest.accountId, [(attributeName): ortb2BlockingAttributeConfig])
        accountDao.save(account)
    ...
    def "PBS should not discard unknown app bids when enforcement is disabled"() {
        given: "Default account with return bid status"
        def bidRequest = BidRequest.defaultBidRequest
    ...
    def "PBS should not discard bids with deals when allowed ortb2 attribute for deals is matched"() {
        ...
        and: "Account in the DB with blocking configuration"
        def attributes = [(attributeName): Ortb2BlockingAttributeConfig.getDefaultOrtb2BlockingAttributeConfig([ortb2Attributes], attributeName, [ortb2Attributes]).tap {
            enforceBlocks = true
        }]
    ...
    def "PBS should discard bids with deals when allowed ortb2 attribute for deals is not matched"() {
        ...
        and: "Account in the DB with blocking configuration"
        def attributeConfig = Ortb2BlockingAttributeConfig.getDefaultOrtb2BlockingAttributeConfig([allowedOrtb2Attributes, dielsOrtb2Attributes], attributeName, [allowedOrtb2Attributes]).tap {
            enforceBlocks = true
        }
    ...
    def "PBS should be able to override blocked ortb2 attribute by bidder"() {
        ...
        def ortb2BlockingAttributeConfig = Ortb2BlockingAttributeConfig.getDefaultOrtb2BlockingAttributeConfig([ortb2Attributes], attributeName).tap {
            enforceBlocks = true
            actionOverrides = Ortb2BlockingActionOverride.getDefaultOrtb2BlockingActionOverride(attributeName, ortb2BlockingOverride, null)
        }
        def account = getAccountWithOrtb2BlockingConfig(bidRequest.accountId, [(attributeName): ortb2BlockingAttributeConfig])
        accountDao.save(account)
...
//    @IgnoreRest
    ...
    def "PBS should be able to override allowed ortb2 attribute for deals by deal ids"() {
        ...
        then: "PBS response should contain only seatbid with proper deal id"
        assert response.seatbid.bid.flatten().size() == 1
        assert getOrtb2Attributes(response.seatbid.first.bid.first, attributeName) == [overrideAttributes]*.toString()
    ...
    private static Account getAccountWithOrtb2BlockingConfig(String accountId, Object ortb2Attributes, Ortb2BlockingAttribute attributeName) {
        getAccountWithOrtb2BlockingConfig(accountId, [(attributeName): Ortb2BlockingAttributeConfig.getDefaultOrtb2BlockingAttributeConfig(ortb2Attributes, attributeName)])
    }
    ...
    private static Bid getBidWithOrtb2Attribute(Imp imp, Object ortb2Attributes, Ortb2BlockingAttribute attributeName) {
        Bid.getDefaultBid(imp).tap {
            switch (attributeName) {
                case BADV:
                    adomain = [ortb2Attributes] as List<String>
                    break
                case BAPP:
                    bundle = ortb2Attributes
                    break
                case BATTR:
                    attr = [ortb2Attributes] as List<Integer>
                    break
                case BCAT:
                    cat = [ortb2Attributes] as List<String>
                    break
                case BTYPE:
                    break
                default:
