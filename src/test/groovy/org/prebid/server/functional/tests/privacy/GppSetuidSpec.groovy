...
class GppSetuidSpec extends PrivacyBaseSpec {
    ...
    def "PBS setUid shouldn't reject request when gpp is invalid"() {
        ...
        where:
        gppSid << [null, "${TCF_EU_V2.value},${USP_V1.value}"]
    }
    ...
    def "PBS setUid shouldn't reject request when gpp doesn't corresponding to gpp_sid"() {
        ...
        where:
        gpp                                                  | gppSid
        new UspV1Consent.Builder().build().encodeSection()   | PBSUtils.getRandomNumberWithExclusion(USP_V1.intValue).toString()
        new TcfEuV2Consent.Builder().build().encodeSection() | PBSUtils.getRandomNumberWithExclusion(TCF_EU_V2.intValue).toString()
    }
