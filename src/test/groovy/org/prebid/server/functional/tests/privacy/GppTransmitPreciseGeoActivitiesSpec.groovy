...
import static org.prebid.server.functional.model.config.LogicalRestrictedRule.generateSolidRestriction
...
class GppTransmitPreciseGeoActivitiesSpec extends PrivacyBaseSpec {
    ...
    def "PBS auction call when privacy regulation don't match custom requirement should not round lat/lon data"() {
        ...
        def accountGppConfig = new AccountGppConfig().tap {
            ...
            it.enabled = true
            it.moduleConfig = ModuleConfig.getDefaultModuleConfig(new ActivityConfig([TRANSMIT_PRECISE_GEO], accountLogic))
        }
    ...
    def "PBS auction call when custom privacy regulation have invalid setup should not round lat/lon data in request with warning and add alert metric"() {
        ...
        def accountGppConfig = new AccountGppConfig().tap {
            ...
            it.config = new SidsConfig().tap { it.skipSids = [] }
            it.enabled = true
            it.moduleConfig = ModuleConfig.getDefaultModuleConfig(new ActivityConfig([TRANSMIT_PRECISE_GEO], restrictedRule))
        }
    ...
    def "PBS auction call when custom privacy regulation with normalizing should change request consent and call to bidder"() {
        ...
        def accountGppConfig = new AccountGppConfig().tap {
            ...
            it.config = new SidsConfig().tap { it.skipSids = [] }
            it.enabled = true
            it.moduleConfig = ModuleConfig.getDefaultModuleConfig(new ActivityConfig([TRANSMIT_PRECISE_GEO], restrictedRule), [USP_CT_V1], true)
        }
        ...
        gppStateConsent
                << [new UspCaV1Consent.Builder()
                            ...
                            .build(),
                    new UspVaV1Consent.Builder().setKnownChildSensitiveDataConsents(PBSUtils.getRandomNumber(0, 2))
                            .setSensitiveDataProcessing(UsVirginiaSensitiveData.generateRandomSensitiveData())
                            .setMspaServiceProviderMode(PBSUtils.getRandomNumber(0, 2))
                            .build(),
                    new UspCoV1Consent.Builder().setKnownChildSensitiveDataConsents(PBSUtils.getRandomNumber(0, 2))
                            .setSensitiveDataProcessing(UsColoradoSensitiveData.generateRandomSensitiveData())
                            .setMspaServiceProviderMode(PBSUtils.getRandomNumber(0, 2))
                            .build(),
                    new UspUtV1Consent.Builder().setKnownChildSensitiveDataConsents(PBSUtils.getRandomNumber(0, 2))
