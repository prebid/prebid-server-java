...
class StoredResponseSpec extends BaseSpec {
    ...
    def "PBS should throw error when imp.ext.prebid.storedBidResponse.seatbidobj is with empty seatbid"() {
        ...
        verifyAll(response.ext.errors[PREBID]) {
            ...
            it.errorMessage == ['Invalid request format: Seat can\'t be empty in stored response seatBid']
        }
    ...
    def "PBS should throw error when imp.ext.prebid.storedBidResponse.seatbidobj is with empty bids"() {
        ...
        verifyAll(response.ext.errors[PREBID]) {
            it.code == [BidderErrorCode.GENERIC]
            it.errorMessage == ['Invalid request format: There must be at least one bid in stored response seatBid']
        }
