...
class MetricsSpec extends BaseSpec {
    ...
    def "PBS should update account.<account-id>.requests metric when verbosity level is basic"() {
        ...
        def metrics = defaultPbsService.sendCollectedMetricsRequest()
        assert metrics[MetricsUtil.Account.requests(accountId)] == 1
        ...
        assert !metrics[MetricsUtil.Account.requestType(accountId, ChannelType.WEB)]
    ...
    def "PBS should update account.<account-id>.* metrics when verbosity level is detailed"() {
        ...
        def metrics = defaultPbsService.sendCollectedMetricsRequest()
        assert metrics[MetricsUtil.Account.bidsReceived(accountId, BidderName.GENERIC)] == 1
        assert metrics[MetricsUtil.Account.prices(accountId, BidderName.GENERIC)] == bidPrice
        assert metrics[MetricsUtil.Account.requestTime(accountId, BidderName.GENERIC)] == 1
        assert metrics[MetricsUtil.Account.requestsGotBids(accountId, BidderName.GENERIC)] == 1
        assert metrics[MetricsUtil.Account.requests(accountId)] == 1
        assert metrics[MetricsUtil.Account.requestType(accountId, ChannelType.WEB)] == 1
    }
    ...
    def "PBS should update hood metrics when bid request contains hood channel type and verbosity level is detailed"() {
        ...
        then: "account.<account-id>.* should be populated"
        def metrics = defaultPbsService.sendCollectedMetricsRequest()
        assert metrics[MetricsUtil.Account.requestType(accountId, ChannelType.DOOH)] == 1
        assert metrics[MetricsUtil.Adapter.requestType(BidderName.GENERIC, ChannelType.DOOH)] == 1

        and: "ather channel types should not be populated"
        assert !metrics[MetricsUtil.Account.requestType(accountId, ChannelType.WEB)]
        assert !metrics[MetricsUtil.Account.requestType(accountId, ChannelType.APP)]
    }
    ...
    def "PBS with soft setup should ignore other distribution channel and update only app metrics when presented app ant other channels in request"() {
        ...
        and: "Metrics processed across site should be updated"
        def metrics = softPrebidService.sendCollectedMetricsRequest()
        assert metrics[MetricsUtil.Account.requestType(accountId, ChannelType.APP)] == 1
        assert metrics[MetricsUtil.Adapter.requestType(BidderName.GENERIC, ChannelType.APP)] == 1

        and: "alert.general metric should be updated"
        assert metrics[MetricsUtil.General.alert()] == 1

        and: "Other channel types should not be populated"
        assert !metrics[MetricsUtil.Account.requestType(accountId, ChannelType.DOOH)]
        assert !metrics[MetricsUtil.Account.requestType(accountId, ChannelType.WEB)]
